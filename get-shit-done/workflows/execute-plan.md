...
Extract from init JSON: `executor_model`, `commit_docs`, `planning_root`, `phase_dir`, `phase_number`, `plans`, `summaries`, `incomplete_plans`, `state_path`, `config_path`.

If planning root missing: error.
</step>
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files ${phase_dir}/{phase}-{plan}-SUMMARY.md ${planning_root}/STATE.md ${planning_root}/ROADMAP.md ${planning_root}/REQUIREMENTS.md
```
</step>
