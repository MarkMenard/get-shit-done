...
const { escapeRegex, normalizePhaseName, comparePhaseNum, findPhaseInternal, getArchivedPhaseDirs, generateSlugInternal, getMilestonePhaseFilter, toPosixPath, planningRoot, output, error } = require('./core.cjs');
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: `${rootRel}/phases/${match}`,
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  const result = {
    ...
    name: description,
    slug,
    directory: `${rootRel}/phases/${dirName}`,
  };
