...
const { escapeRegex, loadConfig, getMilestoneInfo, getMilestonePhaseFilter, planningRoot, output, error } = require('./core.cjs');
...
function cmdStateLoad(cwd, raw) {
  ...
  let stateRaw = '';
  try {
    stateRaw = fs.readFileSync(path.join(root, 'STATE.md'), 'utf-8');
  } catch {}

  const configExists = fs.existsSync(path.join(root, 'config.json'));
  const roadmapExists = fs.existsSync(path.join(root, 'ROADMAP.md'));
